    ...
    if selected_option == "faqs":
        ...
        # Trigger Gemini for "Other Policy Questions"
            if "department" not in context.user_data:
                    ...
                    await query.edit_message_text("Please select your department:", reply_markup=reply_markup)
                    return
        ...
        # Save department
        context.user_data["department"] = selected_option.split("_")[1]
        keyboard = [
            [InlineKeyboardButton("Enter CGPA", callback_data="enter_cgpa")],
            [InlineKeyboardButton("Not Applicable", callback_data="cgpa_na")]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        await query.edit_message_text("Department recorded. Please provide your CGPA for the 1st and 2nd semesters:", reply_markup=reply_markup)
        return
...
@sync_to_async
def get_gemini_response(query, department, cgpa, further_study):
    try:
...
async def handle_policy_question(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    gemini_response = await get_gemini_response(user_message, department, cgpa, further_study)
