...
def get_cohere_response(query):
    try:
        ...
        # Step 1: Search the PolicyFAQ model for relevant answers
        policy_faqs = PolicyFAQ.objects.filter(
            Q(question__icontains=query) | Q(answer__icontains=query) | Q(keywords__icontains=query)
        )

        if policy_faqs.exists():
            # Convert queryset to a list and fetch context
            faq_list = list(policy_faqs)
            context = "\n".join([f"Q: {faq.question} A: {faq.answer}" for faq in faq_list])
        else:
            context = ""  # If no FAQs exist, avoid NoneType issues

        # Step 2: If no relevant answer is found, use Cohere
        if context:
            response = cohere_client.generate(
                model="command",
                prompt=f"Refer to the following policy data and generate a response for '{query}':\n\n{context}\n\n Answer:",
                max_tokens=90,
                temperature=0.7,
                k=0,
                stop_sequences=[],
            ...
            return "I couldn't find a relevant answer in the database. Please refine your question."
